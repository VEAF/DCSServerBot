...
from .dependencies import get_active_sitac
from .foothold import Sitac
from .schemas import MapConnection, MapData, MapEjectedPilot, MapPlayer, MapZone, Server
...
def create_router(plugin: "Foothold") -> APIRouter:
    """Create API router with plugin instance closure.
    ...
    async def foothold_list_servers() -> Any:
        return [
            Server.model_validate({"name": server})
            for server in plugin.list_foothold_servers()
        ]

    @router.get("/{server}/sitac", response_model=Sitac)
    async def foothold_get_sitac(
