...
from plugins.foothold.app.config import (
    AppConfig,
    TileLayerConfig,
    load_config,
    load_config_str,
...
def test_map_config_alternative_tiles() -> None:
    ...
    assert (
        cfg.map.alternative_tiles[0].url
        == "https://tile.openstreetmap.org/{z}/{x}/{y}.png"
    )
    assert cfg.map.alternative_tiles[1].name == "Terrain"
    assert (
        cfg.map.alternative_tiles[1].url
...
def test_map_config_alternative_tiles_env_expansion(
    ...
    assert (
        cfg.map.alternative_tiles[0].url == "https://custom.tiles.org/{z}/{x}/{y}.png"
    )
